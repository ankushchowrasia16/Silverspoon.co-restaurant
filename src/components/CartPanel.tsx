...
export const CartPanel: React.FC<CartPanelProps> = ({ isOpen, onClose, onCheckout }) => {
  const { items, updateQuantity, removeFromCart, clearCart, subtotal, tax, specialDiscount, couponDiscount, total, applyCoupon, couponApplied } = useCart();
  ...
  const specialDiscountPercent = (specialDiscount / subtotal * 100).toFixed(0);
              ...
              {specialDiscount > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-green-600 dark:text-green-400 flex items-center gap-1">
                    ...
                    Special Discount ({specialDiscountPercent}%)
                  </span>
                  <span className="font-medium text-green-600 dark:text-green-400">-{formatPrice(specialDiscount)}</span>
                </div>
              )}
              {couponApplied && couponDiscount > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-green-600 dark:text-green-400 flex items-center gap-1">
                    ...
                    Coupon ({couponApplied}) - 10%
                  </span>
                  <span className="font-medium text-green-600 dark:text-green-400">-{formatPrice(couponDiscount)}</span>
                </div>
              )}
